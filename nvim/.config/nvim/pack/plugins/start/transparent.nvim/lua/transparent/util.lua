local hsluv = require("transparent.hsluv")
...
function util.debug(colors)
  colors = colors or require("transparent.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "transparent" then
    vim.cmd([[autocmd! transparent]])
...
function util.autocmds(config)
  vim.cmd([[augroup transparent]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("transparent.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/transparent/** nested colorscheme transparent]])
  end
...
function util.color_overrides(colors, config)
  if type(config.colors) == "table" then
    for key, value in pairs(config.colors) do
      ...
      -- Patch: https://github.com/ful1e5/transparent.nvim/issues/6
